...
//        nextTrack(true, event);
    ...
    private void calculateNormalized(AudioTrack audioTrack, int defaultVolume) {
        ...
        //get video info
        String result = getUrlData("https://www.youtube.com/get_video_info?video_id=" + videoID +
        ...
        String player_response = null;
        for (String i : result.split("&")) {
            int index = i.indexOf('=');
            String key = i.substring(0, index);
            if (key.equals("player_response"))
                player_response = i.substring(index + 1);
        }
