...
import static com.ice.main.util.PermissionController.removePermission;
import static com.ice.main.util.PermissionERROR.noPermissionERROREmbed;
import static com.ice.main.util.PlaceholderReplacer.placeholderReplacer;
import static com.ice.main.util.Tag.tagRoleID;
        ...
        switch (args[1]) {
            case "newTicket":
                ...
                guild.createTextChannel(
                        ...
                        .addMemberPermissionOverride(member.getIdLong(), Arrays.asList(VIEW_CHANNEL, MESSAGE_SEND), Collections.emptyList())
                        .queue(tc -> {
                            ...
                            addPermission(allowRole, tc, Arrays.asList(VIEW_CHANNEL, MANAGE_CHANNEL, MESSAGE_SEND));
            ...
            case "delC":
                if (member.hasPermission(Permission.MANAGE_CHANNEL)) {
                    ...
                    try {
                        ...
                        System.err.println("Ticket Channel Delete Failed");
                    }
            ...
            case "lock":
                if (member.hasPermission(Permission.MANAGE_CHANNEL) || member.getRoles().contains(guild.getRoleById(args[5]))) {
                    ...
                    removePermission(guild.getRoleById(args[5]), event.getGuildChannel(), Arrays.asList(MESSAGE_SEND, MANAGE_CHANNEL));
                    removePermission(guild.retrieveMemberById(args[4]).complete(), event.getGuildChannel(), List.of(MESSAGE_SEND));
                    String voiceChannelID;
                    VoiceChannel voiceChannel;
                    if ((voiceChannelID = linkedVoiceChannel.get(event.getTextChannel().getId())) != null && (voiceChannel = guild.getVoiceChannelById(voiceChannelID)) != null) {
            ...
            case "uLock":
                if (member.hasPermission(Permission.MANAGE_CHANNEL) || member.getRoles().contains(guild.getRoleById(args[5]))) {
                    ...
                    addPermission(guild.getRoleById(args[5]), event.getGuildChannel(), Arrays.asList(MESSAGE_SEND, MANAGE_CHANNEL));
                    addPermission(guild.retrieveMemberById(args[4]).complete(), event.getGuildChannel(), List.of(MESSAGE_SEND));
                    String voiceChannelID;
                    VoiceChannel voiceChannel;
                    if ((voiceChannelID = linkedVoiceChannel.get(event.getTextChannel().getId())) != null && (voiceChannel = guild.getVoiceChannelById(voiceChannelID)) != null) {
