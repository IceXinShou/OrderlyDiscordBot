...
import com.ice.main.command.list.Ticket;
...
import static com.ice.main.BotSetting.defaultTicketChannelName;
import static com.ice.main.Main.botID;
...
import static com.ice.main.util.JsonGetter.getOrDefault;
import static com.ice.main.util.JsonGetter.getOrDefaultArray;
import static com.ice.main.util.JsonKeys.*;
import static com.ice.main.util.Tag.tagChannel;
...
public record SettingTicket(GuildSettingHelper settingHelper) {
    ...
    public void newTicket(SlashCommandEvent event, boolean newTicket) {
        ...
        if (textCategoryID == null)
            textCategoryID = guild.getVoiceChannelById(sendTicketMessageChannel.getId()).getParentCategoryId();

        if (hasVoiceChannel) {
            if (voiceName == null)
                voiceName = defaultTicketChannelName;
            if (voiceCategoryID == null)
                voiceCategoryID = guild.getVoiceChannelById(sendTicketMessageChannel.getId()).getParentCategoryId();
        }
